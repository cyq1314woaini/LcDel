...
def find_coverage(bam, max_work, output):
    ...
    print("coverageTemp文件夹已存在，不进行创建，直接读取即可计算覆盖度即可~~")
    print("=============================================")
...
def find_sv(bam_path, max_work, outpath, chromosomeslength):
    ...
    # 判断上面文件夹是否存在，存在的话直接去读取即可
        ...
        print("svtemp 文件夹已经存在，直接读取通过cigar查找到的deletion即可")

    print("遍历cigar查找deletion结束~~~")
...
def find_deletion_from_cluster(deletioncluster):
    # 先判断一下最前面和最后面两个长度之间的差异,用来判断ont数据
    ont = False
    if ont:
        firstlens = deletioncluster[0][2]
        lastlens = deletioncluster[-1][2]
        rate = abs(firstlens - lastlens) / max(firstlens, lastlens) * 1.0
        if rate > 0.2:
            return []
...
def merge_small_deletion(deletionset, minsupport):
    ...
    if len(deletionset) >= minsupport:
        if minsupport >= len(deletionset) >= 1.5 * minsupport:
...
def async_cluster_deletion(allsv, outpath, minsupport, maxdepth, chrom, chromlength, path):
    """
    ...
    for lagers in largerdel:
        ...
        for small in smalldeletion:
            ...
            if min(large_end, small_end) - max(large_start,
                                               small_start) > 0 and 0.8 * small_svlen <= large_svlen <= small_svlen / 0.8:
                ovlapflag = 1
                break
        if ovlapflag == 0:
            newSV += [lager]
...
def cluster_deletions(deletionsv, outpath, min_support, highcov, maxwork, chromosomeslength):
    ...
    for chrom in chromosomeslength:
        pool.apply_async(async_cluster_deletion,
                         args=(deletionsv[chrom], outpath, min_support, highcov, chrom, chromosomeslength[chrom], path),
                         error_callback=print_error)
...
def main(bampath, maxwork, outpath, minsup):
    """
    ...
    # 将覆盖度写入文件夹中
    coverage_path = os.path.join(outpath, "coverage.txt")
    cpfile = open(coverage_path, "w")
    cpfile.write(str(depth))
    cpfile.close()
    ...
    vcfpath = os.path.join(outpath, "deletion.vcf")
...
# NA19240
# bampath = "/public_data/SV/long_read/NA19240_bwamem_GRCh38DH_YRI_20160905_pacbio.bam"


# max_work = 20
# outpath = "/home/yuyanan/AssemblySv/clr_test/20x/support"
# minsup = 10
#
# main(bampath, max_work, outpath, minsup)
