...
def async_find_sv_cigar_softclip(chrom, chromlength, bam_path, outpath):
    ...
    for read in allreads:
        ...
        if read.has_tag("SA"):
            ...
            readinfo = [readname, flag, chrom, position, refend, cigarinfo[2], mapq, read.query_sequence, "primary"]
            # print(readinfo)
            ...
            for c in sainfo:
                ...
                sareadinfo = [readname, saflag, sach, sapos, sapos + sacigarinfo[0],
                              sacigarinfo[1], saseq, "segment"]
                segmentreads[readname] += [sareadinfo]

    # 接下来判断clip中是否有代表deletion的
    # clipsv = []
    for readgroup in segmentreads:
        if len(readgroup) < 2 or len(readgroup) > 20:
            continue
